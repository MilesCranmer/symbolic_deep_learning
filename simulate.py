    ...
    def simulate(self, ns, key=0):
        ...
        def make_sim(key):
            if sim in ['string', 'string_ball']:
                ...
                x0.at[s_[..., -1]].set(1) #const mass
                x0.at[s_[..., 0]].set(np.arange(n)+x0.at[...,0].get()*0.5)
                x0.at[s_[..., 2:3]].set(0.0)
            else:
                x0 = random.normal(key, (n, total_dim))
                x0.at[s_[..., -1]].set(np.exp(x0.at[..., -1].get())); #all masses set to positive
                if sim in ['charge', 'superposition']:
                    x0.at[s_[..., -2]].set(np.sign(x0.at[..., -2].get())); #charge is 1 or -1
